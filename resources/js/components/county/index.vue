...
<v-content>
    <v-container fluid fill-height>
        <v-layout justify-center align-center wrap>
            ...
            <v-flex sm12>
                <v-card style="padding: 10px 0;">
                    <v-layout row>
                        <v-flex sm1 style="margin-left: 10px;">
                            <v-tooltip right>
                                <template v-slot:activator="{ on }">
                                    <v-btn icon v-on="on" slot="activator" class="mx-0" @click="getCounty">
                        ...
                        <v-flex sm3>
                            <h3 style="margin-left: 30px !important;margin-top: 10px;">Counties</h3>
                        </v-flex>
                        <v-flex offset-sm8 sm3>
                            <v-btn color="info" @click="openCreate" text>Create County</v-btn>
                        </v-flex>
                    </v-layout>
                </v-card>
            </v-flex>
            ...
            <v-flex sm12>
                <vue-good-table class="table-hover" :columns="columns" :rows="county" :search-options="{ enabled: true }" :pagination-options="{enabled: true,mode: 'pages'}" v-loading="loading" :sort-options="{enabled: true, initialSortBy: {field: 'id', type: 'asc'}}">
...
export default {
    ...
    methods: {
        openCreate() {
            eventBus.$emit("openCreateCounty");
        },
        openEdit(data) {
            eventBus.$emit("openEditCounty", data);
        },
